...
int leftLine = 0;
int rightLine = 0;
int correctLinefactor1 = 0;
int correctLinefactor2 = 0;
int correctLinefactor3=0;
int correctLinefactor4=0;
int Backpathfactor1 = 0;
int Backpathfactor2 = 0;
int Backpathfactor3=0;
int  Backpathfactor4=0;
int rightJunction = 0;
...
void loop() {
  ...
  if (leftJunction == HIGH){
    stop(); //set both motor speed to 0
    delay(1000);
    while (true){
      sharpLeft();
      correctLinefactor1 = digitalRead(rightLineSensorInner);
      correctLinefactor2 = digitalRead(leftLineSensorInner);
      correctLinefactor3 = digitalRead(rightLineSensorOuter);
      correctLinefactor4 = digitalRead(leftLineSensorOuter);
      if ( correctLinefactor1== LOW && correctLinefactor2==LOW && correctLinefactor3==LOW && correctLinefactor4==LOW ){
        break;
        stop();
      }
    }
    ...
    if (Blockcollected == 1){
      while (true){
        sharpRight();
        Backpathfactor1 = digitalRead(leftLineSensorInner);
        Backpathfactor2 = digitalRead(rightLineSensorInner);
        Backpathfactor3 = digitalRead(rightLineSensorOuter);   
        Backpathfactor4 = digitalRead(leftLineSensorOuter);
        if (Backpathfactor1 == HIGH && Backpathfactor2 == LOW && Backpathfactor3 == LOW && Backpathfactor4 == HIGH){
          break;
          stop();
        }
      }
